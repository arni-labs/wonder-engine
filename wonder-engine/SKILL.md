...
Do not imitate or name any existing book, character, author, illustrator, or exact style. Use broad functional ingredients only: dense picture-book worlds, odd science, warm adventure, clear characters, visible systems, and layouts that can become a clean PDF.
...
Prompt it as an exploratory mood-and-world frame: the place, social mix, human/animal/object/nature balance, material texture, density, lighting, and one small visible incident. Ask for feedback on what feels right, wrong, too literal, too generic, too culturally thin, too crowded, too machine-heavy, or worth carrying forward.
...
If multiple probes or tools have been used, record which images are approved style references and which are rejected. Do not reference rejected images in later prompts. If a later image accidentally resembles a rejected reference, explicitly remove that reference direction and regenerate from the approved reference stack.
...
- world anchors: map/geography, routes, scale, material palette, light/weather, recurring objects, natural forces, rituals, tools, machines if relevant, rules, civic systems, source-metaphor logic, language/signage handling, do-not-change notes, and reusable prompt snippet
- approved reference image paths when available, especially the character-introduction and world-introduction spreads
- rejected reference image paths or descriptions when they could cause drift
- production format rules: cover aspect ratio and typography policy; interior artwork aspect ratio; whether spread art should be seamless panoramic art with no generated center fold
...
If a later user change alters a character design, map, object, world rule, approved style reference, rejected style direction, cover typography, or production format rule, update the continuity bible first, then revise affected prompts, images, and text.

If a generated image contains an artifact, such as a missing eye, extra limb, unintended crease, mangled prop, wrong hair color, or accidental world rule, do not let it become canon by inertia. Mark it as non-canonical in the continuity bible unless the human explicitly likes it and asks to adopt it.
...
- rejected style references have not crept back in
