...
def line_height(font: ImageFont.ImageFont) -> int:
    ...
    return max(1, int((bottom - top) * 1.45))
...
def block_height(lines: list[str], font: ImageFont.ImageFont) -> int:
    lh = line_height(font)
    height = 0
    for line in lines:
...
def fit_body_font(
    ...
    for pt in range(start_pt, min_pt - 1, -1):
        ...
        if block_height(lines, font) <= max_height:
...
def draw_centered_lines(
    ...
    for line in lines:
        if line:
            ...
            draw.text((x + max(0, (w - tw) // 2), cursor), line, font=font, fill=fill)
            cursor += lh
        else:
            cursor += int(lh * 0.55)
...
def draw_cover_text(
    ...
    for pt in range(title_start, 22, -2):
        ...
        if block_height(lines, font) <= title_box[3]:
            draw_centered_lines(draw, lines, font, title_box, BLUE)
            break
    ...
    if subtitle:
        ...
        for pt in range(subtitle_start, 11, -1):
            ...
            if block_height(lines, font) <= subtitle_box[3]:
                draw_centered_lines(draw, lines, font, subtitle_box, INK)
                break
    ...
    draw_centered_lines(draw, author_lines, author_font, author_box, INK)
...
def draw_text_block(
    ...
    if str(block.get("vertical_align", "top")) == "center":
        cursor = y + max(0, (h - block_height(lines, font)) // 2)
    ...
    for line_index, line in enumerate(lines):
        ...
        if line:
            ...
            draw.text((draw_x, cursor), line, font=font, fill=fill)
        cursor += step
...
def draw_text_blocks(
    ...
    for block in blocks:
        warnings.extend(draw_text_block(draw, manifest, page, block, dpi))
    return warnings
...
def render_page(manifest: dict, page: dict, base_dir: Path, dpi: int) -> tuple[Image.Image, list[str]]:
    ...
    if page.get("type") == "cover":
        ...
        warnings.extend(draw_text_blocks(draw, manifest, page, dpi))
    else:
